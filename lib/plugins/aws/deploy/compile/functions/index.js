  ...
  compileFunctions() {
    ...
    this.serverless.service.getAllFunctions().forEach((functionName) => {
      ...
      if (newFunction.Properties.VpcConfig) {
        ...
        // merge in the vpcDefaultSecurityGroup
        const vpcDefaultSecurityGroup = this.serverless.utils.readFileSync(
          path.join(this.serverless.config.serverlessPath,
            'plugins',
            'aws',
            'deploy',
            'compile',
            'functions',
            'vpc-default-security-group.json')
        );

        // Security group is required to depend on IAM Policy to force it to be 
        // created between the policy and function
        vpcDefaultSecurityGroup.DependsOn = [ "IamPolicyLambdaVpc" ];

        _.merge(this.serverless.service.provider.compiledCloudFormationTemplate.Resources,
          vpcDefaultSecurityGroup);

        // Function is required to depend on the security group and policy to give enough time for the 
        // IAM policy to be created and replicated
		    newFunction.DependsOn = [ "LambdaFunctionDefaultVpcSecurityGroup", "IamPolicyLambdaVpc"];
      }
