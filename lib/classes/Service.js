    ...
    return that.serverless.yamlParser
      ...
      .then((serverlessYmlParam) => {
        ...
        if (serverlessYml.defaults) {
          ...
          if (serverlessYml.defaults.region) {
            this.defaults.region = serverlessYml.defaults.region;
          }
          if (serverlessYml.defaults.variableSyntax) {
            this.defaults.variableSyntax = serverlessYml.defaults.variableSyntax;
          }
        }

        // Moving defaults into provider obj
        if (serverlessYml.provider.stage) {
          this.defaults.stage = serverlessYml.provider.stage;
        }
        if (serverlessYml.provider.region) {
          this.defaults.region = serverlessYml.provider.region;
        }
        if (serverlessYml.provider.variableSyntax) {
          this.defaults.variableSyntax = serverlessYml.provider.variableSyntax;
        }
